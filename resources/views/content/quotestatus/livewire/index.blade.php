<div class="card">
    <div class="card-header">
        <h4 class="card-title"><a href="{{ route('quotestatus.create') }}" class="btn btn-primary">+ Nieuwe status</a></h4>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody @if ($search === '') wire:sortable="updateOrder" @endif>
                                    @forelse ($quotestatus as $status)
                                        <tr @if ($search ===  '') wire:sortable.item="{{ $status->id }}" @endif wire:key="quote-status-{{ $status->id }}">
                                            @if ($search === '')
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a class="btn btn-primary" href="{{ route('quotestatus.show', $status->id) }}">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a class="btn btn-secondary" href="{{ route('quotestatus.edit', $status->id) }}">
