<div class="card">
    <div class="card-header">
        <h4 class="card-title"><a href="{{ route('invoicestatus.create') }}" class="btn btn-primary">+ Nieuwe status</a></h4>
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody @if ($search === '') wire:sortable="updateOrder" @endif>
                                    @forelse ($invoicestatus as $status)
                                        <tr @if ($search ===  '') wire:sortable.item="{{ $status->id }}" @endif wire:key="invoice-status-{{ $status->id }}">
                                            @if ($search === '')
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a class="btn btn-primary" href="{{ route('invoicestatus.show', $status->id) }}">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a class="btn btn-secondary" href="{{ route('invoicestatus.edit', $status->id) }}">
