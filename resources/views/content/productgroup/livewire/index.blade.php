<div class="card">
    <div class="card-header">
        <h4 class="card-title"><a href="{{ route('productgroup.create') }}" class="btn btn-primary">+ Nieuwe product groep</a></h4>
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody @if ($search === '') wire:sortable="updateOrder" @endif>
                                    @forelse ($product_groups as $product_group)
                                        <tr @if ($search ===  '') wire:sortable.item="{{ $product_group->id }}" @endif>
                                            @if ($search === '')
                                            ...
                                            <td>{{ $product_group->updated_at->format('d-m-Y | H:i') }}</td>
                                            <td>{{ $product_group->created_at->format('d-m-Y | H:i') }}</td>
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a class="btn btn-primary" href="{{ route('productgroup.show', $product_group->id) }}">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a class="btn btn-secondary" href="{{ route('productgroup.edit', $product_group->id) }}">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <button class="btn btn-danger" wire:click="deleteConfirm('{{ $product_group->id }}')">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="100%">
                                                <span class="font-weight-bold">Er zijn geen product groepen gevonden</span>
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
